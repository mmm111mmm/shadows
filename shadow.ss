...
(define (ev l table)
    (letrec ((1st             (lambda (l) (car l)))
             ...
             (4th             (lambda (l) (car (cdr (cdr (cdr l))))))
             (cannot-find-var (lambda (n) (cons 'cannot-find (cons 'n '()))))
             (is-app?         (lambda (l) (not (null? (2nd (1st l))))))
             (evlist          (lambda (l) (cond ((null? l) '()) (else (cons (ev (1st l)  table) (evlist (cdr l)))))))
             (find-in-entry   (lambda (name l not-found-fn)
                                (letrec ((find (lambda (names values)
                                                 (cond ((null? names)          (not-found-fn name))
                                                       ((eq? name (car names)) (car values))
                                                       (else                   (find (cdr names) (cdr values)))))))
                                  (find (car l) (car (cdr l))))))
             (find-in-table   (lambda (name t not-found-fn)
                                (cond ((null? t) (not-found-fn name))
                                      (else (find-in-entry name (car t)
                                                           (lambda (n)
                                                             (find-in-table name (cdr t) not-found-fn)))))))
             (evcond          (lambda (lines t)
                                (cond ((eq? (1st (1st lines)) 'else)     (ev (2nd (1st lines)) t))
                                      ((ev (1st (1st lines)) t) (ev (2nd (1st lines)) t))
                                      (else                              (evcond (cdr lines) t))))))
      (cond ((number? l) l)
            ((eq? l '#f) #f)
            ((eq? l '#t) #t)
            ((atom? l)   (find-in-table l table cannot-find-var))
            (else
             (cond ((eq? (1st l) 'q)       (2nd l))
                   ((eq? (1st l) 'cons)    (cons (ev (2nd l) table) (ev (3rd l) table)))
                   ((eq? (1st l) 'car)     (car (ev (2nd l) table)))
                   ((eq? (1st l) 'cdr)     (cdr (ev (2nd l) table)))
                   ((eq? (1st l) 'add1)    (add1 (ev (2nd l) table)))
                   ((eq? (1st l) 'sub1)    (sub1 (ev (2nd l) table)))
                   ((eq? (1st l) 'number?) (number? (ev (2nd l) table)))
                   ((eq? (1st l) 'zero?)   (zero? (ev (2nd l) table)))
                   ((eq? (1st l) 'null?)   (null? (ev (2nd l) table)))
                   ((eq? (1st l) 'eq?)     (eq? (ev (2nd l) table) (ev (3rd l) table)))
                   ((eq? (1st l) 'atom?)   (atom? (ev (2nd l) table)))
                   ((eq? (1st l) 'cond)    (evcond (cdr l) table))
                   ((eq? (1st l) 'lambda)  (cons 'closure (cons table (cons (2nd l) (cons (3rd l) '())))))
                   ((is-app? l)            (letrec ((closure (ev (1st l) table))
                                                    (vals    (evlist (2nd l)))
                                                    (params  (3rd closure))
                                                    (entry   (cons params (cons vals '()))))
                                             (ev (4th closure) (cons entry table))))
...
(ev '(
      (lambda (x)
        (cond ((eq? x 6)
               (cons x (q ())))
              (else
               (q building-steam-with-a-grain-of-salt))))
      ((add1 y))
     )
 '(((y)(4))) )
